...
bpb<-barplot(BETA.Mercado, main="Cálculo do Beta-Mercado da Carteira Sugerida (60 dias)", ylim=c(0,3.0),names.arg=names, ylab="Percentual %", cex.names=1, col="pink")
text(x = as.vector(bpb), y = BETA.Mercado, 
     label =  round(BETA.Mercado, 2), pos = 3, 
     col = "black",cex=0.8)
```

## Indicadores de Performance Curto Prazo:
...
:::: {style="text-align: justify"}
::: {style="text-align: justify"}
## Conclusão do resultado no Curto Prazo:
...
Na análise de médio prazo, foi definida a janela temporal entre **01/04/2025 e 11/08/2025**. A escolha desse período busca capturar um horizonte intermediário, suficiente para refletir as condições recentes do mercado sem ser excessivamente curto (o que poderia gerar distorções por ruídos de curto prazo), nem muito longo (o que diluiria os efeitos das oscilações mais atuais).
...
selic <- get_series(11, start_date = "2025-04-01",end_date ="2025-08-11");selic 
library(zoo)
Selic=zoo(selic$'11')
index(Selic)=selic$date
Selic;plot(Selic) 
...
# 1) IBOVESPA em formato zoo
ibov_df <- yf_get("^BVSP", first_date = "2025-04-01", last_date = "2025-08-11")
ibov <- zoo(ibov_df$price_adjusted, order.by = ibov_df$ref_date)  # objeto zoo
...
::: {style="text-align: justify"}
A análise do beta de mercado da carteira mostra diferenças relevantes no risco sistemático de cada ativo em relação ao Ibovespa.

O ativo RENT3 apresenta o maior beta (1,29), indicando que tende a ser mais volátil que o mercado, amplificando tanto ganhos quanto perdas. Já a ação LWSA3 (1,17) também apresenta um risco acima da média, mas ligeiramente menor que RENT3.

Por outro lado, BRFS3 apresenta o menor beta (0,62), mostrando-se mais defensiva e menos sensível às oscilações do mercado. ELET6 (0,91) e ITSA4 (0,95) ficam próximos de 1, o que significa que acompanham de forma relativamente equilibrada os movimentos do índice, com risco próximo ao do mercado.

De forma geral, a carteira equilibra ativos mais defensivos (BRFS3) com ativos mais agressivos (RENT3 e LWSA3), além de empresas de risco moderado (ELET6 e ITSA4). Isso pode contribuir para diversificação e equilíbrio no perfil de risco da carteira sugerida.

**Ranking dos ativos por Beta (90 dias):**

1.   **BRFS3 – 0,62** (mais defensiva, menor sensibilidade ao mercado)
...
5.   **RENT3 – 1,29** (mais arriscada, maior sensibilidade ao mercado)
:::
...
bpb<-barplot(BETA.Mercado, main="Cálculo do Beta-Mercado da Carteira Sugerida (90 dias)", ylim=c(0,3.0),names.arg=names, ylab="Percentual %", cex.names=1, col="pink")
text(x = as.vector(bpb), y = BETA.Mercado, 
     label =  round(BETA.Mercado, 2), pos = 3, 
     col = "black",cex=0.8)
```

## Indicadores de Performance de Médio Prazo:

::: {style="text-align: justify"}
Na análise de médio prazo (01/04/2025 a 11/08/2025), observa-se que:

-    **LWSA3** foi o destaque positivo. Apesar do Beta acima de 1 (maior risco sistemático), apresentou **Alpha positivo (Jensen)** e foi o ativo com o **melhor Sharpe** relativo, entregando desempenho mais eficiente em termos de risco-retorno.

-   **RENT3** também se destacou, com o **maior Alpha (0,0148)**, além de resultados relativamente melhores nos índices Treynor e Sharpe, posicionando-se como o segundo melhor ativo.

    **BRFS3** mostrou-se fraca, com o pior resultado no **Jensen (-0,0214)** e no **Treynor (-8,85)**, além de Sharpe negativo, embora ligeiramente melhor que ITSA4 e ELET6.

-    **ITSA4** apresentou desempenho ruim, com Alpha negativo, Sharpe (-4,21) e Treynor também negativo, não entregando retorno ajustado ao risco.

-    **ELET6**, apesar do Beta defensivo, foi a pior escolha geral: destruiu valor em todos os indicadores e não compensou o risco, mesmo sendo teoricamente menos exposta às oscilações do mercado.

**Ranking de desempenho (melhor → pior):**

1.   **LWSA3** → melhor relação risco-retorno relativa, Alpha positivo e destaque no Sharpe.

2.   **RENT3** → maior Alpha, bom desempenho relativo em todos os indicadores.

3.   **BRFS3** → fraco, mas menos destrutivo que ITSA4 e ELET6 em alguns índices.

4.   **ITSA4** → Alpha negativo e fraca relação risco-retorno.

5.   **ELET6** → pior desempenho geral, destruiu valor em todos os indicadores.
:::

::: {style="text-align: justify"}
## Jesen:

O indicador de Jensen (Alpha de Jensen) mede a geração de valor acima ou abaixo do retorno esperado pelo modelo CAPM.

-    **RENT3 (0,0148)** e **LWSA3 (0,013)** foram os únicos ativos a gerar Alpha positivo, entregando retorno acima do esperado e se destacando como os melhores nesse critério.

-   **ITSA4 (-0,0014)** e **ELET6 (-0,0039)** tiveram desempenho levemente negativo, não compensando o risco sistemático assumido.

-    **BRFS3 (-0,0214)** foi o pior ativo nesse indicador, destruindo valor e ficando muito abaixo do retorno esperado.
:::
...
::: {style="text-align: justify"}
O índice de Treynor avalia a eficiência do ativo em gerar retorno em relação ao risco sistemático (Beta).

-    Todos os ativos apresentaram valores **negativos**, indicando que nenhum compensou adequadamente o risco de mercado.

-    **RENT3 (-4,23)** e **LWSA3 (-4,27)** foram os “menos ruins”, sugerindo que, apesar de Beta elevado, destruíram menos valor que os demais.

-    **ELET6 (-5,81)** e **ITSA4 (-5,53)** tiveram performance inferior, não entregando retorno ajustado ao risco sistemático.

-    **BRFS3 (-8,85)** foi o pior ativo, mostrando grande destruição de valor frente ao risco de mercado.
:::
...
::: {style="text-align: justify"}
O índice de Sharpe avalia a relação risco-retorno considerando a volatilidade total do ativo.

-    **LWSA3 (-1,54)** apresentou o melhor resultado relativo, sendo o ativo com a melhor relação risco-retorno, mesmo negativo.

-   **RENT3 (-2,35)** e **BRFS3 (-2,05)** ficaram em posição intermediária, mas ainda com má relação risco-retorno.

-    **ELET6 (-3,37)** e **ITSA4 (-4,21)** foram os piores, indicando alto risco sem retorno proporcional.
:::

## Análise de Longo Prazo:

::: {style="text-align: justify"}
Na análise de longo prazo, foi estabelecida a janela temporal de **02/09/2023 a 30/09/2025**. Essa escolha se justifica por permitir a observação de um horizonte mais amplo, contemplando **dois anos de dados de mercado**. Ao adotar esse intervalo, é possível suavizar ruídos de curto prazo e capturar o comportamento dos ativos em diferentes ciclos do mercado, incluindo momentos de valorização e queda. Isso garante uma avaliação mais robusta sobre o desempenho relativo dos ativos, principalmente em métricas de risco e retorno como Jensen, Treynor e Sharpe.
:::
...
selic <- get_series(11, start_date = "2023-09-02",end_date ="2025-09-30");selic 
library(zoo)
Selic=zoo(selic$'11')
index(Selic)=selic$date
Selic;plot(Selic) 
...
# 1) IBOVESPA em formato zoo
ibov_df <- yf_get("^BVSP", first_date = "2023-09-02", last_date = "2025-09-30")
ibov <- zoo(ibov_df$price_adjusted, order.by = ibov_df$ref_date)  # objeto zoo
...
# 6) Carteira BETA final
CarteiraBETA <- data.frame(na.omit(Ativos), Data1)
names(CarteiraBETA) <- c("lwsa3", "elet6", "itsa4", "rent3", "brfs3", "Ibovespa", "Selic", "Data1") #retirei o , "Data1"
attach(CarteiraBETA)
...
::: {style="text-align: justify"}
No horizonte de longo prazo, observa-se que todos os ativos, com exceção de **ELET6**, apresentaram Beta acima de 1, ou seja, tendem a ser mais voláteis que o mercado.

-   **LWSA3 (1,48)** é o ativo mais arriscado, com alta sensibilidade às oscilações do Ibovespa, ampliando tanto ganhos quanto perdas.

-   **RENT3 (1,37)** também se mostra bastante exposta ao risco de mercado, embora um pouco menos que LWSA3.

-   **ITSA4 (1,04)** e **BRFS3 (1,04)** ficaram muito próximos de 1, acompanhando de forma quase proporcional os movimentos do índice.

-   **ELET6 (0,99)** foi o único ativo com Beta ligeiramente abaixo de 1, posicionando-se como o mais defensivo dentro do conjunto, embora ainda muito próximo do risco médio do mercado.

Portanto, no longo prazo a carteira apresenta predominância de ativos com risco elevado (LWSA3 e RENT3), enquanto os demais (ITSA4, BRFS3 e ELET6) se mantêm alinhados ao mercado, sem grandes características defensivas.

**Ranking dos ativos por Beta (longo prazo):**

1.   **ELET6 – 0,99** (mais defensiva, próxima ao mercado)
...
bpb<-barplot(BETA.Mercado, main="Cálculo do Beta-Mercado da Carteira Sugerida (48 meses- 2 anos)", ylim=c(0,3.0),names.arg=names, ylab="Percentual %", cex.names=1, col="pink")
text(x = as.vector(bpb), y = BETA.Mercado, 
     label =  round(BETA.Mercado, 2), pos = 3, 
     col = "black",cex=0.8)
```
...
::: {style="text-align: justify"}
**Conclusão do resultado no Longo Prazo:**\
Entre os ativos analisados, **LWSA3** foi o destaque, apresentando o melhor conjunto de resultados: Alpha positivo relevante, Treynor menos negativo e Sharpe menos ruim, o que reforça sua eficiência relativa, apesar do Beta elevado (alta sensibilidade ao mercado).

**RENT3** também mostrou desempenho sólido, com Alpha positivo e índices de risco melhores que a média, embora ainda negativos.

Na parte intermediária, **BRFS3** conseguiu manter alpha positivo, mas sua relação risco-retorno não se destacou.

Já **ITSA4** e principalmente **ELET6** foram os piores ativos do período, ambos destruindo valor em Sharpe e Treynor, com ELET6 sendo o destaque negativo, apesar de seu perfil mais defensivo pelo Beta.

**Ranking de desempenho (melhor → pior):**

1.  **LWSA3** → melhor desempenho geral (Alpha alto, menor piora nos indicadores de risco).

2.  **RENT3** → desempenho positivo, Alpha consistente, índices moderados.

3.  **BRFS3** → desempenho intermediário, Alpha modesto, relação risco-retorno fraca.

4.  **ITSA4** → fraco desempenho, Alpha baixo e Sharpe muito ruim.

5.  **ELET6** → pior ativo, Alpha neutro e destruição de valor em Sharpe e Treynor.
:::
...
::: {style="text-align: justify"}
**Jensen (Alpha):**\
Avalia se o ativo entregou retorno acima (ou abaixo) do esperado pelo seu risco sistemático.

-   **LWSA3 (0,0203)** e **RENT3 (0,0155)** apresentaram os melhores alphas, mostrando retorno acima do esperado pelo CAPM.

-   **ITSA4 (0,0025)** e **BRFS3 (0,0028)** tiveram resultados positivos, mas modestos.

-   **ELET6 (0,0001)** praticamente zerou, indicando desempenho neutro.
:::
...
**Treynor:**\
Avalia o retorno em excesso obtido por unidade de risco sistemático (Beta).

-   **LWSA3 (-3,16)** e **RENT3 (-3,40)** foram os menos negativos, ou seja, entregaram a “melhor” relação risco-retorno relativo.

-   **ITSA4 (-4,29)** e **BRFS3 (-4,26)** ficaram no meio da distribuição, mas ainda ruins.

-   **ELET6 (-4,52)** apresentou o pior desempenho, sem compensar o risco assumido.
...
::: {style="text-align: justify"}
**Sharpe:**\
Avalia se os retornos compensam a volatilidade total.

-   **LWSA3 (-1,52)** foi o menos ruim, oferecendo a melhor relação risco-retorno relativa.

-   **BRFS3 (-1,61)** e **RENT3 (-1,87)** ficaram em posição intermediária, mas ainda negativos.

-   **ELET6 (-3,03)** e **ITSA4 (-3,38)** tiveram os piores resultados, indicando alto risco sem retorno proporcional.
:::
